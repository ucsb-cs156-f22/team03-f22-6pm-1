...
describe("UCSBOrganizationsTable tests", () => {
  ...
  test("Has the expected column headers and content for adminUser", () => {
    ...
    const { getByText, getByTestId } = render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <UCSBOrganizationsTable ucsbOrganizations={ucsbOrganizationsFixtures.threeOrgs} currentUser={currentUser} />
        </MemoryRouter>
      </QueryClientProvider>
    ...
    expect(getByTestId(`${testId}-cell-row-0-col-orgTranslationShort`)).toHaveTextContent("Theta Tau");
    expect(getByTestId(`${testId}-cell-row-1-col-orgTranslationShort`)).toHaveTextContent("UCSBDK");
